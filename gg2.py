...
def simulate_gg2(mean, variance, max_val, min_val, server_count):
    st.title("G/G/2")
    ...
    while (sum_prob < 0.999):  # Continue the loop until the sum reaches or exceeds 1
        ...
        normal = (1/(mt.sqrt(variance)*mt.sqrt(2*mt.pi))) * \
            mt.exp(-(i-mean)**2/(2*variance))
    ...
    st.subheader("Averges Calculation")
...
####################################################################
    ...
    server_data = [(start_times[i], end_times[i])
                    for i in range(len(arrival))]
    server_labels = [f'Customer {i+1}' for i in range(len(arrival))]
